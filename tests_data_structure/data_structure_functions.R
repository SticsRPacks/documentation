...
usm_list = function(usm_name) {
  return (list(name = usm_name,date_begin = 286,date_end = 650,finit = "canne_ini.xml",
...
# it takes as parameters only the usm's name for now, later it will take all the usm's param
usm_class = function (usm_name) {
  return(usm_class_definiton(name = usm_name,date_begin = 286,date_end = 650,finit = "canne_ini.xml",
                             ground_name = "solcanne",fstation = "climcanj_sta.xml",
                             fclim1 = "climcanj.1998",fclim2 = "clilmcanj.1999",culturean = 0,
                             nb_plant = 0,simulation_code = 1,plant_dominance_1 = pld_sugarCane1,
...
# it takes as parameters only the usm's name for now, later it will take all the usm's param
usm_dataframe = function(usm_name) {
  return (data.frame("name" = usm_name,"date_begin" = 286,"date_end" = 650,"finit" = "canne_ini.xml",
                     "ground_name" = "solcanne","fstation" = "climcanj_sta.xml",
                     "fclim1" = "climcanj.1998","fclim2" = "climcanj.1999","culturean" = 0,
                     "nb_plant" = 1,"simulation_code" = 0,"plant_dominance_1" = pld_sugarCane1,
                     "plant_dominance_2" = pld_sugarCane2,"observation_file" = "file.obs",
                     "elements_list" = num_list()))
}
...
vector_usm_list = function(usmNumber) {
  ...
  for (i in 1:usmNumber) {
    vec[[i]] <- usm_list(paste("usm_",i,sep=""))
  }
  return(vec)
}
...
vector_usm_class = function(usmNumber) {
  vec = vector("list",usmNumber)
  for (i in 1:usmNumber) {
    vec[i] <- usm_class(paste("usm_",i,sep=""))
  }
  return (vec)
}
...
vector_usm_dataframe = function(usmNumber) {
  vec = vector("list",usmNumber)
  for (i in 1:usmNumber) {
    vec[[i]] <- usm_dataframe(paste("usm_",i,sep=""))
  }
...
vector_usm_datatable_from_usm_dataframe = function(usmNumber) {
  vec = vector("list",usmNumber)
  for (i in 1:usmNumber) {
    vec[[i]] <- data.table(usm_dataframe(paste("usm_",i,sep="")))
  }
  return (rbindlist(vec))
}
...
lapply_usm_list = function(usmNumber) {
  li <- lapply(1:usmNumber,function(x) usm_list(paste("usm_",x,sep="")))
  return (li)
}
...
lapply_usm_class = function(usmNumber) {  
  li <- lapply(1:usmNumber,function(i) usm_class(paste("usm_",i,sep="")))                                   
  return(li)
}
...
# function that extract a sub-list of usm grop by a common criteria
