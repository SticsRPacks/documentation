...
NPA_list_list = function(repetition,struct) {                                                     
  li = list()
...
# the same list or class instanciation (struct)
# NPA stands for Non Pre-Allocated
NPA_list_class = function(repetition,struct) {                                                     
  li = list()
  for (i in 1:repetition) {
    li = append(li,struct$copy())
  }
  return(li)
}
...
PA_vector_list = function(repetition,struct) {
...
# the same list or class instanciation (struct)
# PA stands for Pre-allocated
PA_vector_class = function(repetition,struct) {
  vec = vector("list",repetition)
  for (i in 1:repetition) {
    vec[[i]] = struct$copy()
  }
...
# the same list or class instanciation (struct)
PA_List_list = function(repetition,struct) {  
...
# return a pre allocated list which contains a certain time (repetition)
# the same list or class instanciation (struct)
PA_List_class = function(repetition,struct) {  
  li <- lapply(1:repetition,function(i) struct$copy())                                          
  return(li)
}
