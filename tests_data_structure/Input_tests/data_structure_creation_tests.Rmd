...
Instanciation_methods <- c("NPA Lapply","PA Lapply")
...
for (i in num_elem) {
  ...
  for (j in 1:length(Structures_types)) {
      ...
      set.seed(1)
      test <- eval(parse(text=paste("list_of_usm_",Structures_types[j],"_by_PA_lapply(",i,")",sep="")))
      ...
      benchmark <- microbenchmark(NPA_Lapply= eval(parse(text=paste("list_of_usm_",Structures_types[j],
                                                                "_by_lapply(",i,")",sep=""))),
                                  PA_Lapply = eval(parse(text=paste("list_of_usm_",Structures_types[j],
                                                                "_by_PA_lapply(",i,")",sep=""))),
                                  setup = set.seed(1),
                                  times = 100)
...
for (i in num_elem) {
  ...
  for (j in 1:length(Structures_types)) {
      ...
      Instanciation_methods = c(Structures_types[j],"Lapply")
      ...
      benchmark <- microbenchmark(structure=eval(parse(text=paste("usm_",Structures_types[j],
