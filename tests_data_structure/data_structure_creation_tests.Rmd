...
The results are clear, the pre-allocated instanciation method is far better then the NPA.
...
for (i in num_elem) {
  ...
  for (j in 1:length(Structures_types)) {
      ...
      if (j < 3) {
        ...
        test <- eval(parse(text=paste("usm_",Structures_types[j],"_by_",Structures_types[j],
                                      "(",i,")",sep="")))
        instance_weight <- object.size(test)
