...
We made some primary tests on USMs and especially on what kind of structure is the best to use
with it. We tested 4 structures that are list, reference class, data frame and tibble.
These tests are basic, in the way that they just create a non pre-allocated list,
a pre-allocated vector and a pre-allocated list. Each of them contains the same usm, but have not 
the same length (10 000, 100 000, 1 000 000).
We also tested the copy-on-modifying on each of these structures.

# Constructors

````{r constructors}
## functions file
source("functions_data_structure_v2.R")
